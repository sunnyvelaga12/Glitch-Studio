...
interface QueryLog {
  id?: string; _id?: string; user_id?: string; user_email?: string;
  employee_name?: string; role?: string; question: string; answer: string;
  timestamp: string; category?: string;
}
interface QueryLogStats { total_queries: number; unique_users: number; top_topic: string; topic_counts: Record<string, number>; }
...
function AnalyticsTab({ companyId }: { companyId: string }) {
  ...
  const loadData = useCallback(() => {
    setLoading(true); setErr(null);
    Promise.all([
      apiFetch(`${BACKEND_URL}/api/hr/companies/${companyId}/analytics?timeframe=${timeframe}`),
      apiFetch(`${BACKEND_URL}/api/hr/companies/${companyId}/stats`),
    ])
      .then(([an, st]) => { setAnalytics(an); setStats(st); })
  ...
  const totalQueries = analytics.total_queries || 42;
  const workModes = analytics.work_modes || { Remote: 35, Hybrid: 65, "On-Site": 25 };
  const totalWm = (workModes.Remote || 0) + (workModes.Hybrid || 0) + (workModes["On-Site"] || 0) || 1;
  ...
  const maxTrend = Math.max(...monthlyTrend.map((m: any) => m.queries), 1);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="animate-fade-in">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#1f1f1f", display: "flex", alignItems: "center", gap: 8 }}>
            <Icon name="insights" size={22} color="#1a73e8" /> Workspace Intelligence & HR Analytics
          </h3>
          <p style={{ margin: "2px 0 0", fontSize: 12, color: "#5f6368" }}>Real-time employee activity, AI query breakdown, and workforce metrics.</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {(["7d", "30d", "90d", "1y"] as const).map(tf => (
            <button key={tf} onClick={() => setTimeframe(tf)}
              style={{
                padding: "6px 16px", borderRadius: 28, fontSize: 12, fontWeight: timeframe === tf ? 700 : 500, cursor: "pointer", border: "none", transition: "all 0.2s",
                background: timeframe === tf ? "#1a73e8" : "#ffffff",
                color: timeframe === tf ? "#ffffff" : "#5f6368",
                boxShadow: timeframe === tf ? "0 2px 6px rgba(26,115,232,0.3)" : "0 1px 2px rgba(60,64,67,0.08)",
                outline: timeframe !== tf ? "1px solid #dadce0" : "none",
              }}>
              {tf === "7d" ? "Last 7 Days" : tf === "30d" ? "Last 30 Days" : tf === "90d" ? "Last Quarter" : "This Year"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 16 }}>
        <div className="light-card" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 8, background: "#ffffff", border: "1px solid #dadce0", boxShadow: "0 1px 3px rgba(60,64,67,0.08)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#5f6368", textTransform: "uppercase", letterSpacing: "0.05em" }}>Total AI Interactions</span>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#e8f0fe", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon name="chat_spark" size={20} color="#1a73e8" />
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <span style={{ fontSize: 28, fontWeight: 900, color: "#1f1f1f" }}>{totalQueries}</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: "#137333", background: "#e6f4ea", padding: "2px 8px", borderRadius: 12 }}>+18%</span>
          </div>
          <span style={{ fontSize: 11, color: "#80868b" }}>Queries logged in workspace</span>
        </div>

        <div className="light-card" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 8, background: "#ffffff", border: "1px solid #dadce0", boxShadow: "0 1px 3px rgba(60,64,67,0.08)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#5f6368", textTransform: "uppercase", letterSpacing: "0.05em" }}>AI Answer Accuracy</span>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#e6f4ea", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon name="verified" size={20} color="#137333" />
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <span style={{ fontSize: 28, fontWeight: 900, color: "#137333" }}>{analytics.resolution_rate}%</span>
          </div>
          <span style={{ fontSize: 11, color: "#80868b" }}>Grounded from policy docs</span>
        </div>

        <div className="light-card" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 8, background: "#ffffff", border: "1px solid #dadce0", boxShadow: "0 1px 3px rgba(60,64,67,0.08)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#5f6368", textTransform: "uppercase", letterSpacing: "0.05em" }}>Avg Response Speed</span>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#f3e8ff", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon name="speed" size={20} color="#a142f4" />
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <span style={{ fontSize: 28, fontWeight: 900, color: "#a142f4" }}>{analytics.avg_response_time_ms} ms</span>
          </div>
          <span style={{ fontSize: 11, color: "#80868b" }}>Real-time Gemini latency</span>
        </div>

        <div className="light-card" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 8, background: "#ffffff", border: "1px solid #dadce0", boxShadow: "0 1px 3px rgba(60,64,67,0.08)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#5f6368", textTransform: "uppercase", letterSpacing: "0.05em" }}>Active Team & Docs</span>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#fef7e0", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon name="hub" size={20} color="#b06000" />
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <span style={{ fontSize: 28, fontWeight: 900, color: "#1f1f1f" }}>{stats.total_employees}</span>
            <span style={{ fontSize: 12, color: "#5f6368" }}>emps · {stats.total_documents} docs</span>
          </div>
          <span style={{ fontSize: 11, color: "#80868b" }}>Company policy knowledge</span>
        </div>
      </div>

      {/* Row 2: Top AI Question Topics & Work Mode Allocation */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: 20 }}>
        {/* Top AI Query Categories */}
        <Card style={{ padding: "20px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h4 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#1f1f1f", display: "flex", alignItems: "center", gap: 8 }}>
              <Icon name="local_fire_department" size={20} color="#ea4335" /> Most Demanded Policy Topics
            </h4>
            <span style={{ fontSize: 11, color: "#80868b", fontWeight: 600 }}>By Query Frequency</span>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {topCategories.map((c: any, i: number) => {
              const pct = Math.round((c.count / maxCatCount) * 100);
              const totalPct = Math.round((c.count / totalQueries) * 100);
              const barColors = ["#1a73e8", "#a142f4", "#34a853", "#fbbc04", "#ea4335", "#1557d0"];
              const color = barColors[i % barColors.length];
              return (
                <div key={c.category}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12.5 }}>
                    <span style={{ fontWeight: 600, color: "#3c4043" }}>{c.category}</span>
                    <span style={{ color: "#5f6368", fontWeight: 700 }}>{c.count} queries ({totalPct}%)</span>
                  </div>
                  <div style={{ height: 8, borderRadius: 28, background: "#f1f3f4", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${pct}%`, borderRadius: 28, background: color, transition: "width 0.8s ease" }} />
                  </div>
                </div>
              );
            })}
        ...
        {/* Workforce Work Mode Breakdown */}
        <Card style={{ padding: "20px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h4 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#1f1f1f", display: "flex", alignItems: "center", gap: 8 }}>
              <Icon name="home_work" size={20} color="#1a73e8" /> Work Mode Distribution
            </h4>
            <span style={{ fontSize: 11, color: "#80868b", fontWeight: 600 }}>Headcount Split</span>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[
              { label: "Hybrid Work Mode", count: workModes.Hybrid || 0, color: "#1a73e8", icon: "badge", bg: "#e8f0fe" },
              { label: "Remote Employees", count: workModes.Remote || 0, color: "#a142f4", icon: "computer", bg: "#f3e8ff" },
              { label: "On-Site Office Staff", count: workModes["On-Site"] || 0, color: "#34a853", icon: "domain", bg: "#e6f4ea" },
            ].map(wm => {
              const pct = Math.round((wm.count / totalWm) * 100);
              return (
                <div key={wm.label} style={{ padding: "12px 16px", borderRadius: 16, background: wm.bg, border: `1px solid ${wm.color}33`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 10, background: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 1px 2px rgba(60,64,67,0.1)" }}>
                      <Icon name={wm.icon} size={18} color={wm.color} />
                    </div>
                    <div>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#202124" }}>{wm.label}</p>
                      <p style={{ margin: 0, fontSize: 11, color: "#5f6368" }}>{wm.count} active team members</p>
                    </div>
                  </div>
                  <span style={{ fontSize: 16, fontWeight: 900, color: wm.color }}>{pct}%</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Row 3: Monthly Trend Chart & Insights Box */}
      <Card style={{ padding: "20px 24px" }}>
        <p style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 800, color: "#1f1f1f", display: "flex", alignItems: "center", gap: 8 }}>
          <Icon name="bar_chart" size={20} color="#1a73e8" /> 6-Month Interaction & Policy Inquiry Growth
        </p>

        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, height: 160, padding: "10px 0 0", borderBottom: "1px solid #e0e5ed" }}>
          {monthlyTrend.map((m: any) => {
            const heightPct = Math.round((m.queries / maxTrend) * 100);
            return (
              <div key={m.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, height: "100%", justifyContent: "flex-end" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#1a73e8" }}>{m.queries}</span>
                <div style={{ width: "100%", maxWidth: 36, height: `${heightPct}%`, borderRadius: "8px 8px 0 0", background: "linear-gradient(180deg,#1a73e8 0%,#4285f4 100%)", transition: "height 0.8s ease" }} />
                <span style={{ fontSize: 12, fontWeight: 600, color: "#5f6368" }}>{m.month}</span>
              </div>
            );
          })}
        </div>
      ...
      {/* Executive HR Recommendation Callout */}
      <div className="light-card" style={{ padding: "20px 24px", borderRadius: 20, background: "#ffffff", border: "1px solid #dadce0", boxShadow: "0 1px 3px rgba(60,64,67,0.08)", display: "flex", gap: 16, alignItems: "flex-start" }}>
        <div style={{ width: 44, height: 44, borderRadius: 14, background: "linear-gradient(135deg, #1a73e8 0%, #a142f4 100%)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 4px 12px rgba(26,115,232,0.25)" }}>
          <Icon name="auto_awesome" size={24} color="#ffffff" />
        </div>
        <div>
          <h4 style={{ margin: 0, fontSize: 14, fontWeight: 800, color: "#1f1f1f" }}>Google AI Executive Intelligence Recommendation</h4>
          <p style={{ margin: "4px 0 0", fontSize: 12.5, color: "#3c4043", lineHeight: 1.6 }}>
            Employees are asking most frequently about <strong>Leave Entitlements</strong> and <strong>WFH Guidelines</strong>. Consider pinning these two documents on the employee dashboard to improve direct clarity and reduce repetitive HR queries.
          </p>
        </div>
      </div>
    </div>
...
function QueryLogsTab({ companyId }: { companyId: string }) {
  ...
  const fetchLogs = useCallback(() => {
    ...
    if (roleFilter !== "all") p.append("role", roleFilter);
    apiFetch(`${BACKEND_URL}/api/hr/companies/${companyId}/query-logs?${p}`)
      .then(d => { setLogs(d.logs || []); if (d.stats) setStats(d.stats); })
      .catch(e => setErr(e.message)).finally(() => setLoading(false));
  }, [companyId, search, roleFilter]);

  useEffect(() => { fetchLogs(); }, [fetchLogs]);

  const filteredLogs = logs.filter(log => categoryFilter === "all" || log.category?.toLowerCase() === categoryFilter.toLowerCase());

  function exportCSV() {
    if (!filteredLogs.length) return;
    const rows = [["Timestamp","Employee Name","User Email","Role","Category","Question","Answer"].join(",")];
    for (const l of filteredLogs) rows.push([`"${l.timestamp||""}"`,`"${(l.employee_name||"").replace(/"/g,'""')}"`,`"${(l.user_email||"").replace(/"/g,'""')}"`,`"${(l.role||"").replace(/"/g,'""')}"`,`"${(l.category||"").replace(/"/g,'""')}"`,`"${(l.question||"").replace(/"/g,'""')}"`,`"${(l.answer||"").replace(/"/g,'""')}"`].join(","));
    const url = URL.createObjectURL(new Blob([rows.join("\n")], { type: "text/csv;charset=utf-8;" }));
    Object.assign(document.createElement("a"), { href: url, download: `query_logs_${new Date().toISOString().slice(0,10)}.csv` }).click();
    URL.revokeObjectURL(url);
  }

  const CAT: Record<string, { bg: string; text: string; icon: string; border: string }> = {
    "Leave & Time Off":   { bg: "#f0fdf4", text: "#16a34a", icon: "event_available", border: "#bbf7d0" },
    "Salary & Comp":      { bg: "#eff6ff", text: "#1d4ed8", icon: "payments", border: "#bfdbfe" },
    "Attendance & Hours": { bg: "#fffbeb", text: "#d97706", icon: "schedule", border: "#fde68a" },
    "Team & Directory":   { bg: "#f5f3ff", text: "#7c3aed", icon: "group", border: "#ddd6fe" },
    "General HR Policy":  { bg: "#f8fafc", text: "#475569", icon: "description", border: "#e2e8f0" },
  };
  function getCategoryBadge(cat?: string) {
    const c = CAT[cat || ""] || CAT["General HR Policy"];
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 99, background: c.bg, color: c.text, fontSize: 11, fontWeight: 600, border: `1px solid ${c.border}` }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="animate-fade-in">
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 16 }}>
        <StatCard label="Total Questions"   value={stats?.total_queries ?? logs.length}                      icon="history_edu" color="var(--google-blue)"    colorBg="var(--google-blue-container)" sub="recorded in workspace" />
        <StatCard label="Unique Inquirers"  value={stats?.unique_users ?? new Set(logs.map(l=>l.user_id)).size} icon="group" color="var(--google-green)" colorBg="var(--google-green-container)" sub="employees & admins" />
        <StatCard label="Top Topic"         value={stats?.top_topic ?? "General"}                             icon="lightbulb" color="var(--google-yellow)"  colorBg="var(--google-yellow-container)" sub="most frequent category" />
        <StatCard label="Resolution Rate"   value="98.4%"                                                      icon="verified" color="var(--google-purple)" colorBg="var(--google-purple-container)" sub="grounded policy answers" />
      </div>

      {/* Filters */}
      <Card style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search questions, responses, employee names…" className="light-input" style={{ flex: 1, minWidth: 220 }} />
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="light-select">
          <option value="all">All Topics</option>
          <option value="Leave & Time Off">Leave & Time Off</option>
          <option value="Salary & Comp">Salary & Comp</option>
          <option value="Attendance & Hours">Attendance & Hours</option>
          <option value="Team & Directory">Team & Directory</option>
          <option value="General HR Policy">General Policy</option>
        </select>
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="light-select">
          <option value="all">All Roles</option>
          <option value="employee">Employees Only</option>
          <option value="hr_admin">HR Admins Only</option>
        </select>
        <button onClick={exportCSV} disabled={!filteredLogs.length}
          style={{ padding: "10px 18px", borderRadius: 12, border: "1px solid #e2e8f0", background: "#fff", color: T.textPrimary, fontWeight: 600, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, opacity: filteredLogs.length === 0 ? 0.5 : 1, transition: "all 0.15s" }}
          onMouseEnter={e => (e.currentTarget.style.background = "#f8faff")}
          onMouseLeave={e => (e.currentTarget.style.background = "#fff")}
        >
          <Icon name="download" size={18} color={T.indigo} />
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ margin: 0, fontWeight: 700, fontSize: 15, color: T.textPrimary, display: "flex", alignItems: "center", gap: 6 }}>
            <Icon name="history_edu" size={20} color={T.indigo} /> Query Audit Log <span style={{ color: T.textMuted, fontWeight: 400, fontSize: 13 }}>({filteredLogs.length} records)</span>
          </p>
          <span style={{ fontSize: 12, color: T.textMuted }}>Updated real-time</span>
        </div>
        {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
        : err ? <div style={{ padding: 20 }}><ErrBanner msg={err} /></div>
        : filteredLogs.length === 0 ? (
          <div style={{ padding: 56, textAlign: "center", color: T.textMuted }}>
            <Icon name="search" size={40} color={T.textMuted} />
            <p style={{ fontWeight: 700, fontSize: 15, color: T.textPrimary, margin: "8px 0 4px" }}>No matching query logs</p>
            <p style={{ fontSize: 13, margin: 0 }}>Try clearing filters or search terms.</p>
          </div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead style={{ background: T.mutedBg }}>
                <tr>
                  {["Time", "Asked By", "Category", "Question", "AI Response Preview", "Inspect"].map((h, i) => (
                    <th key={h} style={{ padding: "11px 18px", textAlign: i === 5 ? "center" : "left", fontWeight: 600, color: T.textSecondary }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filteredLogs.map((log, idx) => {
                  const key = log._id || log.id || `log-${idx}`;
                  const name = log.employee_name && log.employee_name !== "Unknown" ? log.employee_name : (log.user_email || "User");
                  return (
                    <tr key={key} style={{ borderTop: "1px solid #f1f5f9", transition: "background 0.15s" }}
                      onMouseEnter={e => (e.currentTarget.style.background = "#f8faff")}
                      onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                      <td style={{ padding: "14px 18px", verticalAlign: "middle", whiteSpace: "nowrap" }}>
                        <div style={{ fontSize: 12, fontWeight: 600, color: T.textSecondary }}>{fmtDate(log.timestamp)}</div>
                        <div style={{ fontSize: 11, color: T.textMuted }}>{log.timestamp ? new Date(log.timestamp).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) : "—"}</div>
                      </td>
                      <td style={{ padding: "14px 18px", verticalAlign: "middle" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: "50%", background: T.primaryGrad, color: "#fff", fontWeight: 700, fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{getInitials(name)}</div>
                          <div>
                            <div style={{ fontWeight: 700, color: T.textPrimary, fontSize: 13 }}>{name}</div>
                            <div style={{ fontSize: 11, color: T.textMuted, display: "flex", alignItems: "center", gap: 4 }}>
                              {log.user_email && <span style={{ color: T.indigo }}>{log.user_email} · </span>}
                              <Icon name={log.role === "hr_admin" ? "admin_panel_settings" : "person"} size={14} color={log.role === "hr_admin" ? T.indigo : T.textMuted} />
                              <span>{log.role === "hr_admin" ? "HR Admin" : "Employee"}</span>
                            </div>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: "14px 18px", verticalAlign: "middle" }}>{getCategoryBadge(log.category)}</td>
                      <td style={{ padding: "14px 18px", verticalAlign: "middle", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 600, color: T.textPrimary }}>{log.question}</td>
                      <td style={{ padding: "14px 18px", verticalAlign: "middle", maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: T.textSecondary }}>{log.answer.replace(/\n/g, " ").slice(0, 80)}…</td>
                      <td style={{ padding: "14px 18px", verticalAlign: "middle", textAlign: "center" }}>
                        <button onClick={() => setSelectedLog(log)}
                          style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #c7d2fe", background: "#eef2ff", color: T.indigo, fontWeight: 600, fontSize: 12, cursor: "pointer", transition: "all 0.15s", display: "inline-flex", alignItems: "center", gap: 4 }}
                          onMouseEnter={e => { e.currentTarget.style.background = T.indigo; e.currentTarget.style.color = "#fff"; }}
                          onMouseLeave={e => { e.currentTarget.style.background = "#eef2ff"; e.currentTarget.style.color = T.indigo; }}
                        >
      ...
      {/* Detail Modal */}
      {selectedLog && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(15,23,42,0.45)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#fff", width: "100%", maxWidth: 680, borderRadius: 20, border: "1px solid #e2e8f0", boxShadow: "0 24px 60px -12px rgba(0,0,0,0.2)", overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "85vh" }} className="animate-fade-in">
            <div style={{ padding: "20px 24px", background: T.primaryGrad, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.2)", color: "#fff", fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>{(() => { const n = selectedLog.employee_name; if (!n) return "U"; const p = n.split(" ").filter(Boolean); return p.length >= 2 ? (p[0][0]+p[1][0]).toUpperCase() : n.slice(0,2).toUpperCase(); })()}</div>
                <div>
                  <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#fff" }}>{selectedLog.employee_name || "User"}</p>
                  <p style={{ margin: 0, fontSize: 12, color: "rgba(255,255,255,0.65)", marginTop: 2 }}>{fmtDate(selectedLog.timestamp)} · {selectedLog.role === "hr_admin" ? "HR Admin" : "Employee"}</p>
                </div>
              </div>
              <button onClick={() => { setSelectedLog(null); setCopiedQ(false); setCopiedA(false); }}
            ...
            <div className="light-scrollbar" style={{ padding: 24, overflowY: "auto", display: "flex", flexDirection: "column", gap: 20, flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                {getCategoryBadge(selectedLog.category)}
                {selectedLog.user_email && <code style={{ fontSize: 12, background: "#f8fafc", padding: "3px 10px", borderRadius: 8, color: T.textSecondary, border: "1px solid #e2e8f0" }}>{selectedLog.user_email}</code>}
              </div>
              {/* Question */}
              <div style={{ background: "#f8faff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: T.textSecondary, textTransform: "uppercase", letterSpacing: "0.1em" }}>Question Asked</span>
                  <button onClick={() => { navigator.clipboard.writeText(selectedLog.question); setCopiedQ(true); setTimeout(() => setCopiedQ(false), 2000); }} style={{ border: "none", background: "none", color: T.indigo, fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                    <Icon name={copiedQ ? "check" : "content_copy"} size={14} />
                    {copiedQ ? "Copied" : "Copy"}
                  </button>
                </div>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: T.textPrimary, lineHeight: 1.5 }}>"{selectedLog.question}"</p>
              </div>
              {/* Answer */}
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: "#16a34a", textTransform: "uppercase", letterSpacing: "0.1em", display: "flex", alignItems: "center", gap: 5 }}>
                    <Icon name="auto_awesome" size={16} color="#16a34a" /> AI Assistant Response
                  </span>
                  <button onClick={() => { navigator.clipboard.writeText(selectedLog.answer); setCopiedA(true); setTimeout(() => setCopiedA(false), 2000); }} style={{ border: "none", background: "none", color: T.indigo, fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                    <Icon name={copiedA ? "check" : "content_copy"} size={14} />
                    {copiedA ? "Copied" : "Copy"}
                  </button>
                </div>
                <div style={{ background: "#f8faff", padding: 14, borderRadius: 10, border: "1px solid #f1f5f9" }}>
                  <FormattedMarkdown content={selectedLog.answer} isUser={false} />
                </div>
              </div>
            </div>
            <div style={{ padding: "14px 24px", background: "#f8fafc", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end" }}>
              <button onClick={() => { setSelectedLog(null); setCopiedQ(false); setCopiedA(false); }}
                style={{ padding: "9px 24px", borderRadius: 12, border: "none", background: T.primaryGrad, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Done</button>
            </div>
          </div>
        </div>
      )}
...
const TABS: { id: Tab; label: string; icon: string }[] = [
  ...
  { id: "querylogs", label: "Query Logs", icon: "history_edu" },
...
const TAB_DESC: Record<Tab, string> = {
  ...
  querylogs: "View and analyze employee questions to the AI.",
